...
- **2026-09-01 update check (complete for focused scope):** The manual-only More-menu command is cancellation-aware and backed by `UpdateCheckService`. MainWindow owns busy state, localized dialogs, and a second trusted-URI validation before shell launch; `Closed` cancels an active request without changing tab/navigation lifecycles.
- **2026-09-01 update check (in progress):** Add a manual-only, cancellation-aware More-menu command backed by `UpdateCheckService`. MainWindow owns busy state, localized dialogs, and a second trusted-URI validation before shell launch. Preserve tab/navigation lifecycles and cancel the request in `Closed`.
...
- **2026-08-30 detachable tabs:** GREEN for source/focused automated scope. Registered windows share a process-local drag coordinator; docking transfers the existing `AppTab`/`Frame` and its `_frameEditors` set exactly once, while an uncancelled outside drop creates an independent taskbar window. Same-window reorder remains the existing list operation, and duplicate file-path suppression is removed from both open paths. Real foreground drag/drop smoke remains pending.
...
| Date | Change | Author |
|---|---|---|
| 2026-09-01 | Added the manual localized update-check lifecycle and trusted release-page launch boundary. | Codex |
...
| 2026-08-30 | Added cross-window live-tab docking/outside detach, source Home recovery, and duplicate same-file tabs. | Codex |
- v5.2.6 duplicate-file tabs keep independent paths: Editor Save As updates only the initiating tab, while `HandleFilePathChanged` remains the broadcast path for a real library rename. Detached pages resolve their containing window instead of `Application.Current.MainWindow`.
