...
> Last updated: 2026-08-20（Hidden Ink/text geometry/FitToCurve synchronization）| Protection: STANDARD
...
- `PageAnnotation`：`Strokes` / `Texts` / `Highlights` / `Images`，以及独立的 `HiddenInks` List。Hidden Ink 不混入普通笔迹，避免普通橡皮或选择操作误改学习遮罩。
...
- `HiddenInkAnnotation`（学习模式）：`Id`、`R/G/B/A`（默认不透明纸张白色）、`Size=28`（DIP）、`RevealDurationMs`（默认 3000ms）和 `Points`（DIP `[x,y]` 点列）。Reveal 的临时显示状态不序列化，文档重开时遮罩始终恢复为隐藏状态。
...
| `PageAnnotation` | `Strokes/Texts/Highlights/Images/HiddenInks` | 注释容器；`HiddenInks` 是独立的学习遮罩集合 |
...
- Model-level synchronization is complete. `Width`/`Height` are backward-compatible, `FitToCurve` preserves old JSON behavior, and Hidden Ink reveal state remains transient.
- Future model changes must preserve existing field names and `Version=1` compatibility behavior.
...
- Hidden Ink 使用自由手绘点列覆盖关键词；颜色和 alpha 由模型保存，默认是纯白/纸张色且 alpha 强制为不透明。
